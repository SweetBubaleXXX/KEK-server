...
class TestFoldres(TestWithRegisteredKey):
    ...
    def test_rename_folder_already_exists(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "post", "/folders/mkdir", json={"path": "/parent/existing_child"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "post",
            "/folders/rename",
            json={"path": "/parent/child", "new_name": "existing_child"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_rename_folder_not_exists(self):
        response = self.authorized_request(
            "post",
            "/folders/rename",
            json={"path": "/nonexistent_folder", "new_name": "renamed_grandparent"},
        )
    ...
    def test_list_folder(self):
        ...
        for i in range(child_count):
            response = self.authorized_request(
                "post",
                "/folders/mkdir",
                json={"path": f"/parent/child_{i}", "recursive": True},
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "get", "/folders/list", headers={"path": "/parent"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_move_folder(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "post", "/folders/mkdir", json={"path": "/destination"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "post",
            "/folders/move",
            json={"path": "/parent/child", "destination": "/destination"},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        old_parent = (
            self.session.query(models.FolderRecord)
    ...
    def test_move_folder_invalid_destination(self):
        response = self.authorized_request(
            "post",
            "/folders/move",
            json={"path": "/parent", "destination": "/parent/child"},
        )
    ...
    def test_delete_folder(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "delete", "/folders/rmdir", headers={"path": "/parent"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.authorized_request(
            "get", "/folders/list", headers={"path": "/parent/child"}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
