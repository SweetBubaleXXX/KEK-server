...
class TestStorageClient(TestWithRegisteredKey, TestWithStreamIteratorMixin):
    ...
    async def test_upload_existing_file(self, request_mock: AsyncMock):
        ...
        existing_file_record = await crud.create_file_record(
            self.session,
            folder_record,
            "filename",
            self.storage_record,
        ...
        await crud.update_record(self.session, existing_file_record)
        ...
        await crud.update_record(self.session, self.storage_record)
        await crud.update_record(self.session, file_record)
    ...
    async def test_delete_folder(self, request_mock: AsyncMock):
        storage_response = StorageSpaceResponse(used=0, capacity=10000000)
        self.__set_request_mock_value(request_mock, storage_response)
        parent_folder = await crud.create_folders_recursively(
        ...
        for name in filenames:
            ...
            await crud.update_record(self.session, file_record)
        await crud.update_record(self.session, parent_folder)

        await StorageClient.delete_folder(self.session, parent_folder)

        found_folder_record = await crud.find_folder(
            self.session, full_path="/parent_folder"
        )
        self.assertIsNone(found_folder_record)
        for name in filenames:
            found_file_record = await crud.find_file(
